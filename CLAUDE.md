...
- Pipeline position is normalizeStage on the stage field and nothing else; a client or won record only leaves that stage through an explicit user action (the PATCH carries explicit: true), never an import, a background job, or a status change. Every lead entering the app passes normalizeLead() (src/lib/leadShape.js); a screen never assumes a field's type.
