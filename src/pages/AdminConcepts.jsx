  ...
  return (
    <Card padding={3} interactive onClick={onOpen} selected={selected} className={`cp-card${compact ? ' cp-card--compact' : ''}`} aria-label={`Open ${p.title}`}>
      ...
      {!compact && <>
        ...
        {thumbs.length > 0 && <Row gap={1} className="cp-thumbs">{thumbs.map(i => <img key={i.id} src={i.link} alt={i.label || ''} className="cp-thumb" loading="lazy" onError={(e) => { e.currentTarget.style.display = 'none'; }} />)}</Row>}
      </>}
    </Card>
  );
}
