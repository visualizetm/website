  ...
  return (
    <Card padding={3} interactive onClick={onOpen} selected={selected} className={`cp-card${compact ? ' cp-card--compact' : ''}`} aria-label={`Open ${p.title}`}>
      ...
      {!compact && <>
        ...
        {thumbs.length > 0 && <Row gap={1} className="cp-thumbs">{thumbs.map(i => <span key={i.id} className="img-fit img-fit--1x1 cp-thumb"><img src={i.link} alt={i.label || ''} width={64} height={64} loading="lazy" decoding="async" onError={(e) => { e.currentTarget.closest('.cp-thumb').style.display = 'none'; }} /></span>)}</Row>}
      </>}
    </Card>
  );
}
...
const cpStyles = `
  ...
  /* An .img-fit box (a declared ratio, the image clipped inside it) rather
     than a bare sized <img>, the same rule every other image on the site
     follows, so a thumbnail can never draw outside its own square. */
  .cp-thumb { width: 64px; border-radius: var(--v-radius-sm); border: 1px solid var(--v-border); background: var(--v-surface-2); flex-shrink: 0; }
  .cp-thumb--sm { width: 36px; }
