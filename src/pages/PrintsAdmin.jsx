...
export default function PrintsAdmin() {
  ...
  const [maintenancePreview, setMaintenancePreview] = useState(() => localStorage.getItem('vz_maintenance_preview') === 'true');
  const envMaintenanceActive = import.meta.env.VITE_MAINTENANCE_MODE === 'true';
  const [expandedShopOrder, setExpandedShopOrder] = useState(null);
  ...
  const toggleMaintenance = useCallback(() => {
    const next = !maintenancePreview;
    localStorage.setItem('vz_maintenance_preview', String(next));
    window.dispatchEvent(new Event('vz-maintenance-change'));
      ...
      <main className="adm-main">
        ...
        {tab === 'forms' && (
          <div className="adm-content">
            <div className="adm-topbar">
              <div>
                <h1 className="adm-title">Client Briefs</h1>
                ...
                {forms.map(form => {
                  ...
                  const FIELD_SECTIONS = [
                    { title: 'About You', rows: [
                      ['Full name', fld.name], ['Business name', fld.businessName],
                      ['Website URL', fld.websiteUrl], ['Email', form.clientEmail],
                    ]},
                    { title: 'Project Overview', rows: [
                      ['Business description', fld.businessDesc], ['Target audience', fld.targetAudience],
                      ['Primary goal', fld.primaryGoal],
                    ]},
                    { title: 'Pages & Content', rows: [
                      ['Pages needed', (fld.pages || []).join(', ')],
                      ['Content ready', fld.contentReady], ['Professional photos', fld.hasPhotos],
                    ]},
                    { title: 'Design Direction', rows: [
                      ['Sites admired', fld.sitesAdmired], ['Competitors', fld.competitors],
                      ['Design style', (fld.designStyle || []).join(', ')],
                      ['Brand colors', fld.hasBrandColors], ['Colors to avoid', fld.colorsToAvoid],
                    ]},
                    { title: 'Platform & Technical', rows: [
                      ['Platform preference', fld.platform], ['Has domain', fld.hasDomain],
                      ['Must-have features', fld.mustHaveFeatures],
                    ]},
                    { title: 'Timeline & Budget', rows: [
                      ['Timeline', fld.timeline], ['Hard deadline', fld.hardDeadline],
                      ['Budget range', fld.budget], ['Additional info', fld.additionalInfo],
                    ]},
                  ];
                  ...
                  return (
                    <div key={form.id} className="adm-panel adm-panel--full" style={{ padding: 0, overflow: 'hidden' }}>
                      <button
                        ...
                        onClick={() => {
                          setExpandedForm(isOpen ? null : form.id);
                          if (!isOpen && (isNew || isEdited)) markFormSeen(form.id);
                        }}
                      >
                        ...
                        <div style={{ display: 'flex', alignItems: 'center', gap: 'var(--space-2)', flexShrink: 0 }}>
                          {isNew && (
                            <span style={{ fontSize: '0.65rem', fontWeight: 800, letterSpacing: '0.06em', padding: '2px 8px', borderRadius: 999, background: 'rgba(212,76,67,0.18)', color: 'var(--brand)', border: '1px solid rgba(212,76,67,0.3)' }}>NEW</span>
                          )}
                          {isEdited && !isNew && (
        ...
        {/* ── Shop tab ─────────────────────────── */}
        {tab === 'shop' && (() => {
          const allOrders = (() => { try { return JSON.parse(localStorage.getItem('vz_print_orders') || '[]'); } catch { return []; } })();
          const shopOrders = allOrders.filter(o => o.source === 'shop');
          const SHOP_STATUSES = [
            { id: 'pending',    label: 'Pending',    color: '#f59e0b' },
          ...
          const updateShopStatus = (orderId, status) => {
            try {
              const all = JSON.parse(localStorage.getItem('vz_print_orders') || '[]');
              const updated = all.map(o => String(o.id) === String(orderId) ? { ...o, status } : o);
              localStorage.setItem('vz_print_orders', JSON.stringify(updated));
            } catch {}
          };
          return (
            <div className="adm-content">
              <div className="adm-topbar">
                <div>
                  <h1 className="adm-title">Shop Orders</h1>
              ...
              {shopOrders.length === 0 ? (
                ...
                <div style={{ display: 'flex', flexDirection: 'column', gap: 'var(--space-3)' }}>
                  {shopOrders.map(order => {
                    const isOpen = expandedShopOrder === order.id;
                    const st = SHOP_STATUSES.find(s => s.id === order.status) || SHOP_STATUSES[0];
                    return (
                          ...
                          <div style={{ display: 'flex', alignItems: 'center', gap: 'var(--space-3)', flex: 1, minWidth: 0 }}>
                            ...
                            <div style={{ minWidth: 0 }}>
                              <p style={{ fontWeight: 700, fontSize: '0.9375rem', color: 'var(--text)' }}>
                                ...
                                <span style={{ fontWeight: 400, fontSize: '0.8125rem', color: 'var(--text-muted)', marginLeft: 8 }}>
                                  {(order.cartItems || []).length} item{(order.cartItems || []).length !== 1 ? 's' : ''}
                                </span>
                              </p>
                              <p style={{ fontSize: '0.78rem', color: 'var(--text-muted)', marginTop: 2 }}>{formatDate(order.date)} · {order.email}</p>
                            </div>
                          </div>
                          <div style={{ display: 'flex', alignItems: 'center', gap: 'var(--space-2)', flexShrink: 0 }}>
                        ...
                        {isOpen && (
                          <div style={{ borderTop: '1px solid var(--glass-border)', padding: 'var(--space-5)' }}>
                            ...
                            <div style={{ display: 'flex', alignItems: 'center', gap: 'var(--space-3)', marginBottom: 'var(--space-5)' }}>
                              <p style={{ fontSize: '0.78rem', fontWeight: 700, color: 'var(--text-muted)', textTransform: 'uppercase', letterSpacing: '0.08em', flexShrink: 0 }}>Status</p>
                              <div style={{ display: 'flex', gap: 6, flexWrap: 'wrap' }}>
                                {SHOP_STATUSES.map(s => (
                                  <button key={s.id} type="button"
                                    onClick={() => updateShopStatus(order.id, s.id)}
                                    style={{
                                      padding: '4px 12px', borderRadius: 999, fontSize: '0.75rem', fontWeight: 700, cursor: 'pointer', fontFamily: 'inherit', border: `1px solid ${s.color}55`,
                                      background: order.status === s.id ? `${s.color}25` : 'transparent', color: order.status === s.id ? s.color : 'var(--text-muted)',
                            ...
                            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: 'var(--space-3)', marginBottom: 'var(--space-5)' }}>
                              {[['Name', order.name], ['Email', order.email], ['Phone', order.phone || '—']].map(([k, v]) => (
                                <div key={k} style={{ background: 'var(--glass-bg)', borderRadius: 8, padding: '10px 12px' }}>
                                  <p style={{ fontSize: '0.65rem', fontWeight: 700, textTransform: 'uppercase', letterSpacing: '0.08em', color: 'var(--text-muted)', marginBottom: 4 }}>{k}</p>
                                  <p style={{ fontSize: '0.875rem', color: 'var(--text)' }}>{v}</p>
                                </div>
                              ))}
                            </div>

                            {/* Cart items */}
                            <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 'var(--space-3)', paddingBottom: 'var(--space-2)', borderBottom: '1px solid var(--glass-border)' }}>
                              <p style={{ fontSize: '0.7rem', fontWeight: 700, textTransform: 'uppercase', letterSpacing: '0.1em', color: 'var(--text-muted)' }}>Items Ordered</p>
                              {order.estimatedSubtotal != null && (
                                <p style={{ fontSize: '0.8rem', fontWeight: 700, color: 'var(--text)' }}>
                                  Est. ${order.estimatedSubtotal.toLocaleString('en-US', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}{order.hasQuoteItems ? ' + quote' : ''}
                                </p>
                              )}
                            </div>
                            <div style={{ display: 'flex', flexDirection: 'column', gap: 'var(--space-3)' }}>
                              {(order.cartItems || []).map((item, i) => (
                                <div key={i} style={{ background: 'var(--glass-bg)', borderRadius: 10, padding: '14px 16px', border: '1px solid var(--glass-border)' }}>
                                  <div style={{ display: 'flex', alignItems: 'baseline', justifyContent: 'space-between', gap: 12, marginBottom: 6 }}>
                                    <p style={{ fontWeight: 700, fontSize: '0.9rem', color: 'var(--text)' }}>{item.productName}</p>
                                    <p style={{ fontWeight: 800, fontSize: '0.875rem', color: item.priceMode === 'quote' || item.priceTotal == null ? '#f59e0b' : 'var(--text)', flexShrink: 0 }}>
                                      {item.priceMode === 'quote' || item.priceTotal == null ? 'Quote' : `$${item.priceTotal.toLocaleString('en-US', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}`}
                                    </p>
                                  </div>
                                  <p style={{ fontSize: '0.8125rem', color: 'var(--text-secondary)', lineHeight: 1.6 }}>{item.label}</p>
                                  {item.vals?.notes && (
          ...
          <div className="adm-content">
            <div className="adm-topbar">
              <div>
                ...
                <p className="adm-subtitle">Site configuration</p>
              </div>
            </div>
            ...
            <div className="adm-panel adm-panel--full">
              <div style={{ padding: 'var(--space-5)' }}>
                <div style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', gap: 'var(--space-5)' }}>
                  <div style={{ flex: 1 }}>
                    <p style={{ fontWeight: 700, fontSize: '1rem', color: 'var(--text)', marginBottom: 6 }}>Maintenance Mode</p>
                    <p style={{ fontSize: '0.875rem', color: 'var(--text-secondary)', lineHeight: 1.55 }}>
                      Displays the "Under Construction" page to all visitors.
                      Use the toggle to preview on this device, or set{' '}
                      <code style={{ background: 'var(--glass-bg)', padding: '1px 7px', borderRadius: 4, fontSize: '0.8rem' }}>VITE_MAINTENANCE_MODE=true</code>
                      {' '}in Vercel for site-wide maintenance.
                    </p>
                  </div>
                  <button
                    type="button"
                    className={`adm-toggle${maintenancePreview ? ' adm-toggle--on' : ''}`}
                    onClick={toggleMaintenance}
                    aria-label={maintenancePreview ? 'Disable maintenance preview' : 'Enable maintenance preview'}
                  >
                ...
                <div style={{ marginTop: 'var(--space-4)', display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 'var(--space-3)' }}>
                  <div style={{ background: 'var(--glass-bg)', borderRadius: 8, padding: '12px 14px' }}>
                    <p style={{ fontSize: '0.65rem', fontWeight: 700, textTransform: 'uppercase', letterSpacing: '0.08em', color: 'var(--text-muted)', marginBottom: 6 }}>Vercel Env Var</p>
                    <p style={{ fontSize: '0.875rem', color: envMaintenanceActive ? '#4ade80' : 'var(--text-secondary)', display: 'flex', alignItems: 'center', gap: 6 }}>
                      <span style={{ width: 7, height: 7, borderRadius: '50%', background: envMaintenanceActive ? '#4ade80' : 'var(--text-muted)', flexShrink: 0 }} />
                      {envMaintenanceActive ? 'Enabled (site-wide)' : 'Not set'}
                    </p>
                  </div>
                  <div style={{ background: 'var(--glass-bg)', borderRadius: 8, padding: '12px 14px' }}>
                    <p style={{ fontSize: '0.65rem', fontWeight: 700, textTransform: 'uppercase', letterSpacing: '0.08em', color: 'var(--text-muted)', marginBottom: 6 }}>This Device</p>
                    <p style={{ fontSize: '0.875rem', color: maintenancePreview ? 'var(--brand)' : 'var(--text-secondary)', display: 'flex', alignItems: 'center', gap: 6 }}>
                      <span style={{ width: 7, height: 7, borderRadius: '50%', background: maintenancePreview ? 'var(--brand)' : 'var(--text-muted)', flexShrink: 0 }} />
                      {maintenancePreview ? 'Preview active' : 'Normal'}
                    </p>
                  </div>
                </div>
...
const admStyles = `
  * { box-sizing: border-box; margin: 0; padding: 0; }
  ...
  .adm-toggle--on .adm-toggle-thumb { transform: translateX(20px); }
`;
