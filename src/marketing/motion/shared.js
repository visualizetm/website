...
/** The element under `ref` matching `selector` whose centre is nearest the
 * vertical centre of the screen carries `data-center`, re-decided on
 * scroll (Site Prompt 9, the phone's sense of focus without pinning; Site
 * Prompt 10 shares it between the business types and the packages). One
 * passive listener, one rAF, a handful of rects. `skip` is a selector that,
 * when present under the ref, means something else decides the centre (the
 * held desktop track writes data-center itself). A data attribute rather
 * than a class because the cards are React elements that re-render their
 * className once on entering the viewport. */
export function useNearestCenter(ref, selector, skip = '') {
  useEffect(() => {
    ...
    const pick = () => {
      raf = 0;
      if (skip && root.querySelector(skip)) return;
