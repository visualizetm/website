 ...
 * @param {number} [props.offset=0] stagger steps to wait before the first child (reading order across columns)
 ...
 */
export default function Stagger({ cap = 8, offset = 0, as: Tag = 'div', className = '', style, children, ...rest }) {
  const [playing, setPlaying] = useState(true);
  const played = useRef(false);
  useEffect(() => {
    if (played.current) return undefined;
    played.current = true;
    const total = durationMs('--v-dur-enter') + (cap + offset) * durationMs('--v-stagger') + 50;
  ...
  return (
    <Tag className={`v-stagger${playing ? ' is-playing' : ''} ${className}`.trim()} style={style} {...rest}>
      {items.map((child, i) => (
        <div key={child.key ?? i} className="v-stagger-item" style={playing ? { animationDelay: `calc(${Math.min(i, cap) + offset} * var(--v-stagger))` } : undefined}>{child}</div>
      ))}
    </Tag>
  );
}
