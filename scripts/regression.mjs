...
for (const width of WIDTHS) {
  ...
  await step(25, 'Create a pack', async () => { await dialog().getByLabel('Title').fill('Regression pack'); await page.waitForTimeout(200); await dialog().getByRole('button', { name: 'Create' }).click({ timeout: T }); await page.waitForTimeout(800); const t = await toastText(); if (!/created/i.test(t)) throw new Error(`toast "${t}"`); await esc(); return t.trim(); });
