...
for (const width of WIDTHS) {
  ...
  await step(10, 'Book a meeting', async () => { const pos = await outcome('Booked', async () => { const d = new Date(); d.setDate(d.getDate() + 2); await dialog().locator('input[type="date"]').fill(d.toISOString().slice(0, 10)); await dialog().locator('input[type="time"]').fill('10:00'); }); return `position ${pos}`; });
  await step(11, 'Log Said no on the last lead', async () => { await dismissToasts(); await page.locator('.cc-outs button', { hasText: 'Said no' }).first().click({ timeout: T }); await expectVisible(dialog(), ''); await dialog().getByRole('button', { name: 'Log' }).click({ timeout: T }); await page.waitForTimeout(400); const t = await toastText(); if (!/Undo/i.test(t)) throw new Error(`no undo toast: "${t}"`); await page.waitForTimeout(900); const summary = await page.locator('.cc-summary').count(); return `undo toast${summary ? ', session summary' : ''}`; });
  ...
  await step(24, 'Concepts list opens a set in the editor', async () => { await goto('/admin/concepts'); await settle(); await page.getByRole('button', { name: /^Open Lead Business 8/ }).first().click({ timeout: T }); await settle(); await expectVisible(page.locator('.ce-dir').first(), ''); const n = await page.locator('.ce-dir').count(); return `${n} directions in the editor`; });
  await step(25, 'Edit a concept set and save it', async () => { await page.getByLabel('Title').first().fill('Regression title'); await page.getByLabel('Title').first().blur(); await page.waitForTimeout(400); await page.getByRole('button', { name: 'Save changes' }).click({ timeout: T }); await page.waitForTimeout(800); const t = await toastText(); if (!/saved/i.test(t)) throw new Error(`toast "${t}"`); return t.trim(); });
  ...
  await step(27, 'Calendar shows today', async () => { await goto('/admin/calendar'); await page.evaluate(() => localStorage.setItem('vz_cal_view', JSON.stringify('day'))); await goto('/admin/calendar'); await settle(); await expectVisible(page.locator('.cal-strip, .cal-week'), ''); const day = await page.locator('.cal-strip').count(); const n = await page.locator(day ? '.cal-row' : '.cal-block').count(); return day ? `${n} events today` : `week view, ${n} blocks`; });
