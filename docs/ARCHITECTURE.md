...
| `/submissions` | **ListSection** | Panel (search, status groups w/ counts, select-all, skeletons, bulk bar) + detail (status pills, contact chips, socials, fields grid, private notes). Shop-order and review submissions are excluded here (Prompt 11): they render in Print Orders and Reviews | GET/PATCH/DELETE submissions, GET export |
