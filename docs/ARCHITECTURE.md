...
- settings documents by _id: rate:review:<hash> {hits[], createdAt, updatedAt} (the review form's per-sender hourly count, the IP hashed, the window rolling), prefs {pushEnabled, emailEnabled}, auth {salt, hash, changedAt}, dashboard {dailyCallTarget, dashboardLayout}, notifications {readIds, lastSeenAt, snoozedUntil, sentReminderKeys, reminders{meetings,callbacks,bills,reviews}}, profile {name, businessHours{start,end}, theme, reduceMotion}, health {enrichment, scraper, crons, stripe, lastBackupAt}, login-limit {hits{ip: [timestamps]}}, client-log {items[]{kind,message,stack,url,at,ua,receivedAt}, capped at 500}.
