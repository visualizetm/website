...
| Data | apiFetch (src/shared/api.js) to same-origin /api/* Vercel functions, cookie auth, no custom header; the service worker keeps the last successful GET of the five main lists for offline reading (never a public endpoint) |
...
| Auth | One admin password, the ADMIN_PASSWORD constant in api/_lib/config.js (an ADMIN_PASSWORD env var overrides it), constant time compare, ten failures per IP per fifteen minutes (api/_lib/limit.js); a signed 30 day `vz_admin` cookie (`${expiresAt}.${hmac}`, SESSION_SECRET, required on Vercel) verified with no database lookup, no renewal, no CSRF header (SameSite=Lax) (api/_lib/auth.js). The audit is docs/SECURITY-AUDIT.md |
...
src/
  ...
  shared/                  semantics (enums), pricing (catalog), dates, phone, format, api (apiFetch, cookie only), copy (every empty and error string), log (the client error log), color
...
| /clients | Clients: list, LeadDetail in client mode (projects, payments, retainer, deliverables). `?filter=active` is the sidebar's Projects entry and `?filter=planner` (posts to approve) its Planner entry: the same screen with a filter, read from the URL | pages/AdminClients.jsx, components/ClientWorkspace.jsx |
| /orders | Print Orders | pages/AdminOrders.jsx |
| /concepts | Concepts: every concept set across every lead, in the order they need Rob (changes requested, viewed and unanswered, sent, drafts, approved) | pages/AdminConcepts.jsx |
| /leads/:id/concepts | The Concepts editor for one record at any stage (rounds, directions, items, send, feedback, revision rounds); `?set=` picks the round. Reached from the record's Concepts button, the Studio list and a notification | pages/AdminConceptsEditor.jsx, lib/concepts.js |
...
Navigation is one list in src/shell/nav.js, rendered by the sidebar as four collapsible groups in the order a client is landed (Pipeline: Dashboard, Leads, Call Console, Booked, Calendar; Clients: Clients, Projects, Planner; Studio: Print Orders, Concepts, Reviews, Landing; System: Submissions, Recently Deleted, Design, Settings). Each group is a disclosure with its own combined badge, its open state persisted per device (`vz_side_groups`), only the active screen's group open the first time; collapsed to the rail each group is one icon with a tooltip naming its items and a menu. Above the groups a pipeline strip shows Leads, Contacted, Booked, Clients with the conversion between each (src/lib/leads.js `pipelineFunnel`, the same numbers the Dashboard shows), each tapping through to that stage. The Clients badge counts stage client plus won; Projects counts open non-retainer projects; Planner counts posts in review. Redirects: /prints and /admin/prints on the admin host go to /orders; /portal and /intake/* on the public host go to /contact?from=portal (the retired portal notice); /work and /work/:slug redirect permanently to /clients and /clients/:slug (Site Prompt 3); /prints and /prints/* redirect permanently to / (Site Prompt 6 removed the print shop; hand-entered print orders in the admin are unaffected). The public site is /, /services (reachable but unlinked), /clients, /clients/:slug, /contact, /book, /lead-partner, /start, /review and /review/:slug (the review form, unlinked from anywhere on the site: it is a link Rob texts after a delivery, so it carries a noindex meta, robots.txt disallows it, and it is not in the sitemap), and the maintenance screen when VITE_MAINTENANCE_MODE is true. The header links Home, Clients and Contact plus one Book a free call button straight to Calendly.
...
|---|---|---|
| /api/admin/call-leads | GET (?deleted=1), POST (single or leads[]), PATCH ({id,set}, restore; a record whose stage is client or won only leaves it when the body carries `explicit: true`, otherwise 409 and untouched), DELETE (?id, ?ids, ?purgeDeleted=1) | AdminApp, AdminCalls, AdminLeads, AdminSettings, useOptimisticPatch |
| /api/admin/leads/import | POST rows[] (a row matching a client or won record never resets its callStatus or priority; nothing here writes stage) | LeadImport |
...
| /api/admin/concept-packs | GET (?leadId, ?industryKey, ?kind), POST, PATCH. Kept only so the two stored packs stay readable; the admin no longer loads it (docs/CONCEPTS-AUDIT.md) | nothing |
| /api/admin/concept-sets | GET (?leadId, or every live set), POST (mints the token), PATCH ({ id, set }; regenerate: true revokes; draft to sent stamps sentAt), DELETE (tombstone) | AdminApp, AdminConcepts, the concepts editor |
...
| /api/admin/login, logout, session | POST (ten failures per IP per fifteen minutes, 4KB body), POST, GET (verifies the cookie, no renewal) | AdminApp, AdminCalls |
...
| /api/concepts | Rewritten to /api/showcase?r=concepts, so it rides on the showcase function (api/_routes/concepts-public.js). GET (?token) is exactly { client: { displayName }, set: { title, round, intro, status, approvedDirectionId }, directions: [{ id, name, rationale, items: [{ id, kind, image, caption }] }], feedback: [{ at, directionId, action, name }] }, never a note; the first view moves sent to viewed and lastViewedAt is stamped at most hourly. POST (?token) { action: approve, change or note, directionId, note, name }: approve only from sent, viewed or changes; change needs a note and several may land before an approval; note appends only. No token, a malformed, unknown, draft, archived or deleted one, and a directionId from another set (checked inside the query) all answer the same 404 { error: 'not found' }; 30 actions per token per hour under `rate:concepts:<sha256(token)>`, reads never limited; every response is `Cache-Control: no-store`. Test: scripts/concepts-endpoint-test.mjs | The client's concepts link (src/pages/Concepts.jsx, on the Scene engine, standalone with ScrollRoot) |
| /api/planner | GET (?token, ?month) reads one client's month (each post: id, date, time, platforms as an array, format, imageUrl, caption, hashtags, status, note, clientNote); POST (?token) { postId, action: approve or request-change, note } is the only write a client can make. No token, an unknown token, a disabled planner, and a post belonging to someone else all answer the same 404 { error: 'not found' }; 30 actions per token per hour, counted in settings under `rate:planner:<sha256(token)>`; every response is `Cache-Control: no-store` | The client's own planner link (built in planner prompt 2) |
| /api/submissions | POST (public: start, contact, review, shop-order; shop orders also create an orders document). A review carries { name, business, rating 1-5, text, email (optional), slug } and is rate limited to 3 an hour per sender (every other type to 10), counted in settings under `rate:review:<sha256(ip)>` and `rate:form:<sha256(ip)>`; `fields` is normalised to a flat map of 40 string values; a filled `company` field is the honeypot and is answered 200 and dropped | Start.jsx, Review.jsx (the shop that used to post shop-order submissions was removed in Site Prompt 6; the endpoint and the parser stay for old submissions and for orders entered by hand) |
...
- A client always carries clientSince: the call-leads PATCH stamps it the moment a record moves to client or won without one, the POST stamps it on a record created as a client (Add client), and the Won dialog sends its own. Older records are backfilled by the daily cron.
- Pipeline position is decided in one place: `normalizeStage(lead)` in src/shared/semantics.js, on the stage field (a record with no stage and a booked call reads as booked, the legacy fallback; an empty stage reads as lead, which is what the daily cron's heal and the PATCH guard exist for). Nothing derives it from callStatus, the call log, or anything else. scripts/pipeline-test.mjs holds the guard.
- Every record entering the app passes `normalizeLead()` (src/lib/leadShape.js, re-exported by src/lib/leads.js): strings where a screen prints text, arrays where it maps, objects where it reads keys, enums checked, so one malformed record (a JSON string where an array should be, an object where a string should be, a null) renders as a lead with a missing field instead of the ErrorBoundary. LeadCard and LeadDetail apply it again. scripts/repair-leads.mjs applies the same rules to the stored records, report first.
...
- concept_packs: title, leadId, industryKey, kind, prompts[]{id,label,text}, images[]{id,label,link}, tags[], notes, usedFor[], lastUsedAt, archived, createdAt, updatedAt. Retired as a UI (docs/CONCEPTS-AUDIT.md); the collection and its sanitize() stay.
- concept_sets: one presentation sent to a client. leadId, title (120), round (1 up), intro (600), status (draft | sent | viewed | approved | changes | archived), token (minted server side, base64url of 24 random bytes, never accepted from a request; regenerate: true revokes), tokenCreatedAt, lastViewedAt (stamped at most hourly by the public read), sentAt, directions[]{id,name (80),rationale (600),items[]{id,kind (logo | board | mockup | social | web | print | other),image (safeUrl),caption (200),order},order} (1 to 6 directions, up to 12 items each), feedback[]{id,at,directionId or null,action (approve | change | note),note (1000),name (80)} (appended only by the public endpoint), approvedDirectionId, approvedAt, projectId (optional, a revision round's project), archived, deleted, createdAt, updatedAt. Whitelist: api/_routes/concept-sets.js sanitize(). Enums: CONCEPT_SET_STATUSES, CONCEPT_ITEM_KINDS, CONCEPT_FEEDBACK_ACTIONS in src/shared/semantics.js, ids mirrored in api/_semantics.js.
...
- posts: one scheduled social post. leadId (call_leads _id), month "YYYY-MM", date "YYYY-MM-DD", time "HH:MM", platforms (array of 1 to 4 platform ids, deduped, order preserved), platform (the pre-array single id, still accepted and still written as platforms[0] for one release), format (portrait or story, missing reads as portrait), imageUrl, caption (2200), hashtags (500, normalised to single spaced tokens each with one leading #, deduped), status (making, review, approved, posted), note (Rob's, 500), clientNote (the client's, 500), clientNoteAt, approvedAt, postedAt, order, archived, deleted, deletedAt, createdAt, updatedAt. Whitelist: api/_routes/posts.js sanitize(). month, date and time are matched against their formats, so a malformed value stores as empty rather than as junk. Reads go through src/lib/posts.js (platformsOf, formatOf, hashtagsOf, missingForReview): nothing reads post.platform directly. A post can only go out for approval when it has what its format needs, an image for a story and an image, a caption and hashtags for a portrait post.
...
- settings documents by _id: rate:planner:<sha256(token)>, rate:review:<sha256(ip)>, rate:form:<sha256(ip)>, rate:login:<sha256(ip)> {hits[], createdAt, updatedAt} (the four rolling window limiters, api/_lib/limit.js; the key is always a hash, never the token or the address; none of them are in the backup), prefs {pushEnabled, emailEnabled}, dashboard {dailyCallTarget, dashboardLayout}, notifications {readIds, lastSeenAt, snoozedUntil, sentReminderKeys, reminders{meetings,callbacks,bills,reviews}}, profile {name, businessHours{start,end}, theme, reduceMotion}, health {enrichment, scraper, crons, stripe, lastBackupAt}, client-log {items[]{kind,message,stack,url,at,ua,receivedAt}, capped at 500, not in the backup}.
...
- daily (06:00 UTC): rolls retainer.nextBillAt, extends retainer schedules to six future months, cancels retainers past their notice, puts back stage client on any live record whose stage is not a stage but carries client evidence (api/_lib/pipeline.js: clientSince, a won outcome, a published showcase, a project, a purchase, a planner switched on, a testimonial; the nightly enricher has been seen to write stage as ''), counting each heal on the record (stageHeals) and naming it in health.crons.daily.healedRecords, which the notifications drawer shows as a System item ("X was restored to Clients", and after more than two heals, that something upstream is still wiping it), stamps clientSince on any client or won record missing it (the earliest of its first purchase, first project, or updatedAt; scripts/backfill-client-since.mjs runs the same rule on demand, report first), writes the health document.
